    ...
    def pre_train_process(self, sess, data):
        ...
        for epoch in range(self.args.gen_pre_epoch_num):
            while batched_data != None:
                ...
                print("Generator Step: " + str(global_step), end='\r')
...
#         sample_num = int(len(data.data["test"]["sent"])/self.args.batch_size+1) if self.args.test_sample == None else self.args.test_sample
        ind = 0
        for _ in range(1):
            print(str(ind), end='\r')
            ind = ind + 1
